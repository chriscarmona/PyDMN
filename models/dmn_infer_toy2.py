...
class dmn_toy2( pyro.nn.PyroModule ):
    ...
    def __init__( self, edgelist, directed=False, weighted=False, H_dim=3, sigma_k_prior_param=None, random_kernel=False, jitter=1e-3, init_mcmc=False ):
        ...
        self.gp_mean_loc = None
        self.gp_mean_scale = None
        ...
        if init_mcmc:
            ...
            self.gp_mean_ini = torch.zeros( (self.V_net,self.H_dim,self.K_net,self.n_dir,self.n_w) )
    ...
    def model(self):
        ...
        gp_mean = gp_mean.reshape(self.V_net,self.H_dim,self.K_net,self.n_dir,self.n_w)
        gp_coord_demean = gp_coord_demean.reshape(self.V_net, self.H_dim, self.K_net, self.n_dir, self.n_w, self.T_net)

        # Latent coordinates
        gp_coord = gp_mean.expand(self.T_net, self.V_net, self.H_dim, self.K_net, self.n_dir, self.n_w).permute(1,2,3,4,5,0) + gp_coord_demean

        ### Linear Predictor ###
        Y_linpred = torch.einsum('uhkwt,vhkwt->uvtkw', gp_coord[:,:,:,0,:,:], gp_coord[:,:,:,self.n_dir-1,:,:])
    ...
    def guide(self):
        ...
        self.gp_mean_scale = pyro.param("gp_mean_scale", 0.1*torch.ones((self.V_net,self.H_dim,self.K_net,self.n_dir,self.n_w)), constraint=constraints.positive)
        ...
        with pyro.plate('gp_coord_all', self.V_net*self.H_dim*self.K_net*self.n_dir*self.n_w ):
            ...
            pyro.sample( "gp_mean", dist.Normal( self.gp_mean_loc.reshape(self.V_net*self.H_dim*self.K_net*self.n_dir*self.n_w),
                                                self.gp_mean_scale.reshape(self.V_net*self.H_dim*self.K_net*self.n_dir*self.n_w) ) )
    ...
    def init_guide(self):
        ...
        gp_mean = torch.tensor(hmc_samples['gp_mean']).reshape(num_samples,self.V_net,self.H_dim,self.K_net,self.n_dir,self.n_w)
